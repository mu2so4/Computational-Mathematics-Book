...
\begin{example}
	...
	же отрезке $[-4,2]$ на тех же узлах $\{-4,-2,0,2\}$.

	Начнём с $\omega(x)=(x^2-4)(x^2+4x)=x^4+4x^3-8x^2-16x$. Попробуем найти экстремумы этого
	многочлена:
	\[\omega'(x)=4x^3+12x^2-8x-16=(x+1)(x+1+\sqrt 5)(x+1-\sqrt 5).\]
	...
	\[\omega(-1)=1-4-4+16=9,\]
	\[\omega(-1\pm\sqrt 5)=\big((1\mp 2\sqrt 5 + 5)-4\big)
	\big((1\mp 2\sqrt 5 + 5)-4\pm 4\sqrt 5\big)=\]
	\[=(2\mp 2\sqrt 5)(2\pm 2\sqrt 5)=-16.\]

	Таким образом, $||\omega(x)||_{C[-4,2]}=16$.

	Точно найти максимум модуля функции $f^{(3)}(x)=e^x+2\cos x$ не получится,
	поэтому попробуем его оценить. Нижняя оценка очевидна: $f^{(3)}(x)>-2$.
	Грубо оценим сверху. Очевидно, что максимум достигается в точке 2, тогда
	$f^{(3)}(2)=e^2+\cos 2\le 7.4$:
	\[||R_3(x)||_{C[-4,2]}\le\frac{7.4\cdot 16}{24}\approx 4.93.\]
\end{example}
