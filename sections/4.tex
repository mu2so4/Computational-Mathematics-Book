...
\begin{lemma}
	Если разностный оператор $\Lambda_h$ аппроксимирует с $k$-м порядком
	$n$-ю производную, а $T_{mh}=T_m$ -- оператор сдвига, то $T_m\circ
	\Lambda_h$ тоже аппроксимирует с $k$-м порядком $n$-ю производную.
\end{lemma}

\begin{proof}
	Разложим в ряд Тейлора по $h$ при $h=0$ выражение $T_m\circ y(x)$:
	\[T_m\circ y(x)=y(x+mh)=y(x)+O(h),\]
	то есть $T_m=E+O(h)$. Тогда
	\[T_m\Lambda_h[y(x)]=T_m\circ\big(y^{(n)}+O(h^k)\big)=y^{(n)}+O(h^k).\]
\end{proof}
