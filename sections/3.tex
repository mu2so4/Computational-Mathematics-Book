...
\begin{figure}[ht]
	...
	\begin{tikzpicture} [
		...
		\begin{axis} [
			...
			\addplot[color=blue, line width=.08cm]{f(x)};
			\addplot[draw=none,name path=B] {0};
			\foreach \ind in {0,...,\divCou} {
				\pgfmathsetmacro{\xx}{\varA + \ind * \Step}
				...
				%\addplot[gray, fill opacity=0.5] fill between
				%	[of=A and B,soft clip={domain=\xx:\xS}];
				%\tikzfillbetween[of=A and B,
				%	domain=\xx:\xS]{gray, opacity=0.5};
			};
	...
	\begin{tikzpicture} [
		...
		\begin{axis} [
			...
			\foreach \ind in {0,...,\divCou} {
				...
				\pgfmathsetmacro{\xS}{\xx+2*\Step/3}
				...
				\addplot[color=black, thick, domain=\xx:\xT,
					name path=A]
					{newton(x,\xx,\xF,\xS,\xT)};
				%\addplot[gray, fill opacity=0.5] fill between
				%	[of=A and B,soft clip={domain=\xx:\xT}];
				%\tikzfillbetween[of=A and B,
				%	domain=\xx:\xT]{gray, opacity=0.5};
			};
		...
		\end{axis}
	\end{tikzpicture}
	\caption*{Составные формулы в сравнении (заливка потом будет)}
\end{figure}
