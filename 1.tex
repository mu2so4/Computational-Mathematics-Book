...
\begin{example}[графическая интерпретация метода простой итерации]
	...
	\begin{tikzpicture}
		\begin{axis} [
			...
			\addplot[color=red, line width=.08cm]{x+2};
	...
	\begin{tikzpicture}
		\begin{axis} [
			...
			\addplot[color=red, line width=.08cm]{x};

			%\addplot[mark=*,only marks, fill=white] (-2,-1.8647)
			%	node[above, pos=1]{};
			...
			\draw[dashed,black] (-2,0) -- (-2,-2);
			\draw[dashed,black] (-1,0) -- (-1,-1.6321);
			...
			\node[scale=0.6] at (-2,0.04) {$-2$};
			\node[scale=0.6] at (-1,0.04) {$-1$};
			\node[scale=0.6] at (-1.841,0.042) {$\widetilde{x_1}$};
			\node[scale=0.6] at (-1.5,0.04) {$x_0$};
			\node[scale=0.6] at (-1.7769,0.04) {$x_1$};
		\end{axis}
	\end{tikzpicture}
	\newline
\end{example}
